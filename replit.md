...
- FY24-25 totals: the authoritative card figure is ₹341.14 Cr (₹3,411,433,805) — confirmed by two independent sources: the `sale_line` DB register and the State Head Sale 2025-26 sheet filtered on FY2024-25. The item-wise SALE tab (PRODUCT_GROUP-mapped codes only) returns ₹341.73 Cr (₹3,417,311,917), which is ₹0.59 Cr higher and is the outlier; it is used only as the completeness-guard fallback when the DB register holds fewer than 12 months. The ₹0.59 Cr gap is a known reconciling item (the SALE tab includes rows the register does not). The SALE tab control total 3,417,311,917 is the seed baseline and the fallback; 3,411,433,805 is the card anchor. Orders come from the per-month MONTHLY tabs, not the formula-driven Combined tab (which is not cached in the export).
...
- Targets: the Target Master sheet is read-only like every other Sheets source. `lib/mgmt/targets.ts` reads the targets tab; `POST /targets` is removed and returns 405. Pro-rata split preview (`GET /targets/split-preview`) and reads (`GET /targets`) remain. Report target columns: monthly = override else annual/12; missing targets render blank/grey, never zero.
...
- Sanity baselines (DB-derived, as of July 2026): Q1 26-27 primary = ₹73.09 Cr (730,867,205), Q1 invoices 5,714, Q1 distinct customers 439. Analytics YoY (comparable months Apr–Jun): overall −1.4%, Territory +8.5%, Institutional −54.1%. Note: "correct" anchors assuming FY2025-26 fully restored are Q1 25-26 = ₹74.56 Cr, overall −2.0%, Territory +7.8%, Institutional −54.1% (unchanged). The gap is the FY2025-26 primary register shortfall of ₹1.62 Cr / 0.45%, which is within the 1% health-check tolerance and is closed against the materiality bar. FY2025-26 DB total = ₹359.52 Cr (anchor ₹361.14 Cr); FY2024-25 DB = ₹341.14 Cr (matches anchor to rounding). Top head FY25-26 Sandeep Ji ₹164.22 Cr (45.5%).
- FY2023-24 secondary ORDER BOOKING combined tab: 16,533 rows / ₹40.6 Cr (₹105.3 M). This is NOT a duplicate or union of the monthly tabs — it is the reconciling total and must be treated as a separate, additive data point. Do not deduplicate it against the monthly order booking tabs.
- Secondary dashboard freeze (FY2025-26 and older): closed FYs (`isClosedFy(fy) = true`) load from `secondary_dashboard_snapshots` DB table on cold restart and cache permanently (no TTL). On first cold start after the table was created, a Sheets read populates the snapshot; every subsequent restart uses DB only. FY2025-26 secondary: 216 members, plan ₹379.75 Cr, salesReceived ₹240.14 Cr, ytdAchievement 68.1% (loaded Jul 20 2026). The snapshot is now in DB — this FY is frozen.
...
- July month-close checklist (run on or after Aug 1 once max(invoice_date for Jul-26) reaches 2026-07-31): (1) Update `artifacts/api-server/config/verify_anchors.json` — add `"Jul-26"` to `primary_anchors["2026-27"].closedMonths` and set `closedMonthsTotal` to the sum of Apr+May+Jun+Jul from `sale_line WHERE fy='2026-27'`. (2) Run `GET /api/audit` to confirm Jul-26 passes the closed-months dispatch check. (3) Secondary side auto-updates on next `loadStateDashboard("2026-27")` call once state heads have entered July figures in the STATE HEAD DASHBOARD. (4) Run `GET /api/customers/months?fy=2026-27` to confirm July has moved from `partialMonths` to `completeMonths`. No code changes needed — the calendar-based `isMonthClosed` and `isMonthComplete` mechanisms handle the flip automatically.
