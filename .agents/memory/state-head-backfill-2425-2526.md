...
**Attribution APPLIED (Aug 4 2026):** the 141 file-matched NULL-state customers were written to sale_line_all (state_canon + head_canon; PROJECT/OTHER→Non-territory bucket, KARNATAKA→'KARNATAKA (B)', heads via head_alias vocabulary — no new state/head values, guarded in-transaction). Residual: ₹3.93 Cr / 104 customers deliberately left NULL ("state not attributed") — user rule: never infer from name parentheticals; resolve via business. Territory (head_canon) figure visibly dropped ₹340.34→₹303.86 Cr because NULL-head rows had counted as territory. Anchor 145,613 / ₹361.00 Cr unchanged; startup freeze assertion passes. Remember: after any direct SQL relabel of a frozen FY, delete route_payload_snapshot keys LIKE '%<fy>%' and restart the server (in-memory snapshot cache).
