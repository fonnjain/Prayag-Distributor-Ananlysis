...
- **C4 suggestion layer is server-only**: suggestions are built solely from already-guarded figures — skip any matrix row with `rankEligible === false` (guard 7) before making a median-relative suggestion, and roster-context suggestions must fire whenever rosterChanges exist, even without quadrants. Cohort suggestions always carry `basis.readings` as caveats.
