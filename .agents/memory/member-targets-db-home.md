...
- **Targets snapshot completeness:** never persist `/targets` before the State Head Dashboard plan load completes. A background-only load can turn valid plans into nulls that the editor presents as zero; serve the last complete snapshot or fail explicitly.
