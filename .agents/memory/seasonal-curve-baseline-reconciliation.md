...
**How to apply:** Prefer a complete nonblank channel classification (`Retail`) over territory flags; if no channel exists, require explicit `is_territory=TRUE` on every contributing row. Any NULL/partial historical attribution remains visible in provenance and verification but blocks multi-year activation; retain the verified FY2025–26 baseline instead. Persist the chosen basis alongside rows/nets, then append an equal-weighted multi-year version only after every frozen year is comparable.