...
- **Shared filter helper:** the entity-filter SQL (entityConds, normStateExpr, prior-FY customer resolution) lives in a standalone saleLineFilter lib so Products/Growth/Company Reports share exact semantics without circular imports (companyReports ↔ analytics).
- **Never use a `\u0000` sentinel in SQL params** — Postgres rejects null bytes in string parameters (500s). An empty prior-FY resolution sets a `none: true` flag that compiles to a literal `false` condition instead.
- Growth/analytics: filtered requests always read the sale_line register even when the SAP aggregate is verified — SAP has no per-line entity dimensions; Momentum's order-book aggregates carry only month+group dims, so it gets export-only (no entity filters).
