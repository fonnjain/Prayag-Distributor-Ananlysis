...
**How to apply:** For any FY-over-FY mix/share claim, run the three-way split — (a) same head both years, (b) head changed, (c) new customer — and report group (b) net before signing off. bool_or over NULL comparisons silently misbuckets: use COALESCE(bool_or(...), false) and treat `head_canon IS NULL` explicitly as territory. A reusable channel-mover grouping exists for the SKU trend surface — reuse it for new cross-FY mix surfaces, keeping its candidate population aligned with the exact level/scope predicates of the comparison it discloses.
