...
- **Month is authoritative from the invoice date (SAP col C), not the user's picker.** The accumulator derives each row's month via `dateToMonthLabel(row.date)`; a dated row outside the requested month is excluded from aggregation and counted in `audit()`. `/sap/register` rejects (422) any upload with off-month rows, naming the detected months, so a mislabeled or mixed-month file cannot silently distort monthly stats. Undated rows fall back to the selected month (historical/manual files may omit a date). **Why:** the per-month drag/drop UX lets a user pick the wrong month; trusting the picker corrupted the analytics.
