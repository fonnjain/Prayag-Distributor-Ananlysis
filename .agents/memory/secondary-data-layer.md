...
- brand-level FY2026-27: mirrored 1:1 into `secondary_register_line` (source='pscode3_brand_rollup', line_uid='brl-'+sku uid) via `scripts/pscode3-brand-backfill.ts` (Aug 2026) — Segment values match prior-FY brand_canon vocabulary exactly, head_canon=head_raw per register convention. D3 segment spread, win-back and effective discount now work for Apr–Jun 2026; live-year guards removed from distributorSkuSpread/skuSpread/distributorInvestment/winBack. Re-run the backfill after any pscode3-load re-run. Item-code features still gate on `secondarySkuFyHasData()`.
