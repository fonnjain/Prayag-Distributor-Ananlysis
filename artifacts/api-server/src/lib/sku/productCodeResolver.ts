...
// This resolves a register code to a master code using a STRICT 5-step order.
...
//   5. Remove internal hyphens, accepting only a unique catalogue match.
//
...
export type ResolveRule = Exclude<ResolveMethod, "ambiguous" | "unresolved">;
...
export type ResolveResult = {
  ...
  /** Every transformation used, in application order. Exact has ["exact"]. */
  appliedRules: ResolveRule[];
  /** Colour letter captured by the colour-suffix step, if any. */
  colour: string | null;
  /** Catalogue candidates when a normalization is not unique. */
  ambiguousMasterCodes: string[];
};
...
export function resolveProductCode(
  ...
  if (has(code)) {
    return { registerCode, masterCode: code, method: "exact", appliedRules: ["exact"], colour: null, ambiguousMasterCodes: [] };
  }
  ...
  if (pm) {
    const base = pm[1];
    if (has(base)) {
      return { registerCode, masterCode: base, method: "p_strip", appliedRules: ["p_strip"], colour: null, ambiguousMasterCodes: [] };
    }
  }
  ...
  if (cm) {
    ...
    if (base.length > 0 && COLOUR_LETTERS.has(colour) && has(base)) {
      return { registerCode, masterCode: base, method: "colour_suffix", appliedRules: ["colour_suffix"], colour, ambiguousMasterCodes: [] };
    }
  }
    ...
    if (target.length > 0) {
      for (const m of masterCodes) {
        if (stripWhitespace(m) === target && m !== code) {
          return { registerCode, masterCode: m, method: "whitespace", appliedRules: ["whitespace"], colour: null, ambiguousMasterCodes: [] };
        }
      }
    }
  }

  // Step 5 — remove internal hyphens. Compare both the direct de-hyphenated
  // form and the whitespace-normalized form so composed matches retain both
  // transformations in their trace. Never choose the first candidate.
  if (masterCodes && code.includes("-")) {
    const directTarget = stripHyphens(code);
    const whitespaceTarget = stripWhitespace(directTarget);
    const directMatches = new Set<string>();
    const whitespaceMatches = new Set<string>();
    for (const master of masterCodes) {
      if (stripHyphens(master) === directTarget && master !== code) directMatches.add(master);
      if (
        stripWhitespace(stripHyphens(master)) === whitespaceTarget
        && master !== code
        && (stripWhitespace(code) !== code || stripWhitespace(master) !== master)
      ) {
        whitespaceMatches.add(master);
      }
    }
    const matches = directMatches.size > 0 ? directMatches : whitespaceMatches;
    if (matches.size === 1) {
      const masterCode = [...matches][0];
      const appliedRules: ResolveRule[] = directMatches.size > 0 ? ["hyphen"] : ["whitespace", "hyphen"];
      return { registerCode, masterCode, method: "hyphen", appliedRules, colour: null, ambiguousMasterCodes: [] };
    }
    if (matches.size > 1) {
      return {
        registerCode,
        masterCode: null,
        method: "ambiguous",
        appliedRules: directMatches.size > 0 ? ["hyphen"] : ["whitespace", "hyphen"],
        colour: null,
        ambiguousMasterCodes: [...matches].sort(),
      };
    }
  }

  return { registerCode, masterCode: null, method: "unresolved", appliedRules: [], colour: null, ambiguousMasterCodes: [] };
}
