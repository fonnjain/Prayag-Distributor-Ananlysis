...
import {
  ...
  buildResolverIndex,
  type ResolveMethod,
} from "../sku/productCodeResolver.js";

import path from "node:path";
function latestProductCsv(): string {
  if (process.env.PRODUCT_CSV) return process.env.PRODUCT_CSV;
  for (const dir of [
    path.resolve(process.cwd(), "attached_assets"),
    path.resolve(process.cwd(), "../../attached_assets"),
  ]) {
    if (!fs.existsSync(dir)) continue;
    const matches = fs.readdirSync(dir).filter((f) => f.startsWith("Product_Upload_Sample_File_") && f.endsWith(".csv")).sort();
    if (matches.length > 0) return path.join(dir, matches[matches.length - 1]);
  }
  throw new Error("Product_Upload_Sample_File_*.csv not found in attached_assets");
}
...
export interface ProductLoadResult { dryRun: boolean; variants: number; codes: number; ok: boolean; }
export async function runProductUploadLoad(opts: { write: boolean; endPool?: boolean }): Promise<ProductLoadResult> {
  const DRY = !opts.write;
  const CSV_PATH = latestProductCsv();
  ...
  if (!countsOk) {
    ...
    return { dryRun: DRY, variants: 0, codes: 0, ok: false };
  }
  ...
  if (opts.endPool) await pool.end();
  return { dryRun: DRY, variants: variants.length, codes: codeAggs.length, ok: true };
}
