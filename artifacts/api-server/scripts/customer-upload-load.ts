...
import { normSecKey } from "../src/lib/mgmt/names.js";
import { normDistKey } from "../src/lib/mgmt/distributorDeepDive.js";

// ── paths ────────────────────────────────────────────────────────────────────
const ATTACHED = path.resolve(process.cwd(), "../../attached_assets");
const DIST_CSV = path.join(ATTACHED, "Distributer_Upload_Sample_File_1786182410874.csv");
const RET_CSV = path.join(ATTACHED, "Retailer_Upload_Sample_file_1786182410891.csv");
const HR_ROSTER = path.resolve(process.cwd(), "config/hr_roster.csv");
const HR_FALLBACK = path.join(ATTACHED, "Sales_User_List_1786182410892.csv");

// --dry-run parses + validates both files but never touches the DB. Use it to
// prove the up-front validation gate works without risking the master.
...
function loadRosterNormKeys(): Set<string> {
  ...
  let src: string;
  try {
    buf = readFileSync(HR_ROSTER);
    src = HR_ROSTER;
  } catch {
    buf = readFileSync(HR_FALLBACK);
    src = HR_FALLBACK;
  }
