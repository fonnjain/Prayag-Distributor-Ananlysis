...
import { CRM_USER_HEAD_MAP_AUTHORITY, CRM_USER_HEAD_MAP_SOURCE } from "./seed/crmUserHeadAuthority.js";

const CRM_USER_HEAD_MAP_SEED_JSON = JSON.stringify(
  CRM_USER_HEAD_MAP_AUTHORITY,
).replaceAll("'", "''");
...
Any published figure labelled only ''Visits'' is ambiguous unless it also identifies aggregate visits, unique visited retailers, or retailer-row visits.',
  ...
  {
    id: "133_crm_user_head_map_authority",
    sql: `
      CREATE TABLE IF NOT EXISTS crm_user_head_map (
        id                  INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        employee_id         TEXT NOT NULL,
        sales_user_name     TEXT NOT NULL,
        employee_id_key     TEXT NOT NULL,
        sales_user_name_key TEXT NOT NULL,
        ...
        created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
        updated_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
        CONSTRAINT crm_user_head_map_valid_dates
          CHECK (effective_to IS NULL OR effective_to > effective_from),
        CONSTRAINT crm_user_head_map_canonical_heads
          CHECK (state_head IN (
            'Sandeep Dadheech', 'Syed Aqil Rizvi', 'Lalan Kumar',
            'Anant Singh', 'Biju C.O', 'Pawan Kumar Sharma',
            'Sunil Patel', 'Sulinder Pal', 'Nasir Hussain Khan'
          ))
      );
      DO $$
      BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM pg_constraint
          WHERE conname = 'crm_user_head_map_valid_dates'
            AND conrelid = 'crm_user_head_map'::regclass
        ) THEN
          ALTER TABLE crm_user_head_map
            ADD CONSTRAINT crm_user_head_map_valid_dates
            CHECK (effective_to IS NULL OR effective_to > effective_from);
        END IF;
        IF NOT EXISTS (
          SELECT 1 FROM pg_constraint
          WHERE conname = 'crm_user_head_map_canonical_heads'
            AND conrelid = 'crm_user_head_map'::regclass
        ) THEN
          ALTER TABLE crm_user_head_map
            ADD CONSTRAINT crm_user_head_map_canonical_heads
            CHECK (state_head IN (
              'Sandeep Dadheech', 'Syed Aqil Rizvi', 'Lalan Kumar',
              'Anant Singh', 'Biju C.O', 'Pawan Kumar Sharma',
              'Sunil Patel', 'Sulinder Pal', 'Nasir Hussain Khan'
        ...
        END IF;
      END $$;
      CREATE UNIQUE INDEX IF NOT EXISTS crm_user_head_map_key_uq
        ON crm_user_head_map (employee_id_key, sales_user_name_key, effective_from);
      CREATE INDEX IF NOT EXISTS crm_user_head_map_lookup_idx
        ON crm_user_head_map (employee_id_key, sales_user_name_key, effective_from);
      CREATE INDEX IF NOT EXISTS crm_user_head_map_head_idx
        ON crm_user_head_map (state_head, effective_from);
      CREATE INDEX IF NOT EXISTS crm_user_head_map_source_idx
        ON crm_user_head_map (source_sha256);
      CREATE OR REPLACE FUNCTION reject_overlapping_crm_user_head_map()
      RETURNS trigger LANGUAGE plpgsql AS $$
      BEGIN
        IF EXISTS (
          SELECT 1
          FROM crm_user_head_map existing
          WHERE existing.employee_id_key = NEW.employee_id_key
            AND existing.sales_user_name_key = NEW.sales_user_name_key
            AND existing.id <> COALESCE(NEW.id, -1)
            AND daterange(existing.effective_from, existing.effective_to, '[)')
                && daterange(NEW.effective_from, NEW.effective_to, '[)')
        ) THEN
          RAISE EXCEPTION 'Overlapping CRM user-head authority for % / %',
            NEW.employee_id, NEW.sales_user_name;
        END IF;
        RETURN NEW;
      END $$;
      DROP TRIGGER IF EXISTS crm_user_head_map_no_overlap ON crm_user_head_map;
      CREATE TRIGGER crm_user_head_map_no_overlap
        BEFORE INSERT OR UPDATE OF employee_id_key, sales_user_name_key,
          effective_from, effective_to
        ON crm_user_head_map
        FOR EACH ROW EXECUTE FUNCTION reject_overlapping_crm_user_head_map();

      INSERT INTO crm_user_head_map
        (employee_id, sales_user_name, employee_id_key, sales_user_name_key,
         state_head, basis, source_file, source_sha256, source_row_number,
         effective_from, effective_to)
      SELECT seed."employeeId",
             seed."salesUserName",
             upper(btrim(seed."employeeId")),
             regexp_replace(upper(btrim(seed."salesUserName")), '[^A-Z0-9]+', '', 'g'),
             seed."stateHead",
             seed.basis,
             '${CRM_USER_HEAD_MAP_SOURCE.sourceFile}',
             '${CRM_USER_HEAD_MAP_SOURCE.sourceSha256}',
             seed."sourceRowNumber",
             DATE '${CRM_USER_HEAD_MAP_SOURCE.effectiveFrom}',
             NULL
        FROM jsonb_to_recordset('${CRM_USER_HEAD_MAP_SEED_JSON}'::jsonb)
          AS seed("employeeId" TEXT, "salesUserName" TEXT, "stateHead" TEXT,
                  basis TEXT, "sourceRowNumber" INTEGER, "augValue" NUMERIC)
